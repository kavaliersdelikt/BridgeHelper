...
public class FeedbackRenderer {
    ...
    // 1.14-1.19.4: HudRenderCallback signature is (MatrixStack, float)
    private void onHudRender(MatrixStack matrices, float tickDelta) {
        ...
        render(matrices, elapsed);
    }

    private void render(MatrixStack matrices, long elapsed) {
        String text;
        ...
        if (settings.shadowEnabled) {
            mc.textRenderer.drawWithShadow(matrices, text, -textWidth / 2f, -textHeight / 2f, finalColor);
        } else {
            mc.textRenderer.draw(matrices, text, -textWidth / 2f, -textHeight / 2f, finalColor);
        }

        int extraY = textHeight / 2 + 2;
        if (settings.showRawDeltaTime) {
            mc.textRenderer.drawWithShadow(matrices,
                    "Delta: " + lastResult.deltaTime + "ms", -textWidth / 2f, extraY, 0xAAFFFFFF);
            extraY += 10;
        }
        if (settings.showIdealMs) {
            mc.textRenderer.drawWithShadow(matrices,
                    "Ideal: " + String.format("%.1f", lastResult.idealMs) + "ms", -textWidth / 2f, extraY, 0xAAFFFFFF);
            extraY += 10;
        }
        if (settings.showAvgSpeed) {
            mc.textRenderer.drawWithShadow(matrices,
                    "Speed: " + String.format("%.2f", lastResult.error), -textWidth / 2f, extraY, 0xAAFFFFFF);
            extraY += 10;
        }
        ...
        if (settings.tickVisualization) {
            int barW = 40;
            int barH = 2;
            DrawableHelper.fill(matrices, -barW / 2, extraY, barW / 2, extraY + barH, 0x55FFFFFF);
            int markerPos = (int) ((lastResult.error / 50.0) * (barW / 2f));
            markerPos = Math.max(-barW / 2, Math.min(barW / 2, markerPos));
            DrawableHelper.fill(matrices, markerPos - 1, extraY - 1, markerPos + 1, extraY + barH + 1, finalColor);
        }

        if (settings.sprintIndicator && showSprintIcon) {
            String sprintChar = "[S]";
            int sprintW = mc.textRenderer.getWidth(sprintChar);
            mc.textRenderer.draw(matrices, sprintChar, -sprintW / 2f, extraY + 5, finalColor);
        }
